...
def generate(force: bool = False) -> int:
    ...
    prompt = (
        ...
        "Rules:\n"
        "- Each fact must be a single sentence, 45-100 characters, in English.\n"
        ...
        # SIMPLE ENGLISH: regola aggiunta il 2026-08-12 su richiesta dell'owner.
        # Il pubblico dei Short e' in larga parte NON madrelingua inglese: un fatto che
        # non si capisce al primo ascolto e' un fatto sprecato (lo spettatore scrolla).
        ...
        "'tiny living things' not 'microbes', 'clear' not 'transparent', 'poop' not "
        "'droppings', 'wings' not 'wingspan'.\n"
        "  * If a hard word IS the fact (octothorpe, tardigrade, cornea, axolotl), keep it "
        "but explain it in the same sentence: 'the cornea, at the front of your eye, ...'.\n"
        "  * One idea per sentence. No subordinate clauses and no -ing clause hanging at "
        "the end ('..., giving them a greenish tint' -> rewrite as a plain sentence).\n"
        "  * Active voice: 'A navy engineer invented the Slinky', not 'The Slinky was "
        "invented by a naval engineer'.\n"
        "  * NO idioms, no figurative language: never 'on the fly', 'collision course', "
        "'clocked at', 'no end in sight', 'hold a grudge', 'standstill to highway speed'.\n"
        "- Conversational and punchy: contractions are fine; prefer concrete numbers.\n"
        "- Spell out EVERY unit and symbol so the voice reads it naturally: 'kilometers', "
        "'kilometers per hour', 'centimeters', 'kilograms', 'degrees Celsius', 'percent' "
        "(never 'km', 'km/h', 'cm', 'kg', 'C', '%', '#', 'deg').\n"
